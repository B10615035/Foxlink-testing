    ...
    def shift__start_time(self, shift_time,before_sf_bt=300,before_sf_rt=120,after_sf_bt=120):
        ...
        for message in self.data["foxlinkEvent"]:
            if message["project"] in ["D52","N104", "X61", "D7X"]:
                message["start_time"] = (shift_time-timedelta(seconds=before_sf_bt)).strftime("%Y-%m-%d %H:%M:%S") # "填入"機台異常開始時間
                if message["status"] == "update":
                    message["end_time"] = (shift_time-timedelta(seconds=before_sf_rt)).strftime("%Y-%m-%d %H:%M:%S") # "填入"機台異常結束時間
            else:
                message["start_time"] = (shift_time+timedelta(seconds=after_sf_bt)).strftime("%Y-%m-%d %H:%M:%S") # "填入"機台異常開始時間
        return self.data
        ...
        for i in [5,10,15,30,60,600]:
            ...
            for idx in range(event_num):
                ...
                if idx%2 == 0:
                    ...
                    time_idx = int(idx/2)
                    ls = [freq[time_idx].strftime("%Y-%m-%d %H:%M:%S"),freq[time_idx+1].strftime("%Y-%m-%d %H:%M:%S")]
                ls_all.append(ls)
